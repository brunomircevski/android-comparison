...
Figure \ref{fig-domains-chart} presents the domain-level distribution of network traffic for Scenarios A and B, selected as the most realistic configurations incorporating essential Google functionality while differing in account linkage and privacy-related settings.

Stock Android contacts the widest range of Google domains, spreading traffic across many different endpoints. Alternative systems show less Google communication, though patterns vary by design. GrapheneOS makes more requests than microG-based systems because it downloads Google services on demand from its own app store rather than having them preinstalled. IodéOS and LineageOS with microG reach fewer Google servers overall, as microG replaces some Google functions with its own infrastructure.

Table \ref{tab:domains-mechanisms} shows a gradient in how much each system relies on Google-operated endpoints. LineageOS (all variants) stays closest to stock Android, using google servers for these basic features. IodéOS sits in the middle, while GrapheneOS pushes furthest away from Google services. Both iodéOS and GrapheneOS also surface relevant toggles in their system settings, letting users decide for themselves whether to enable or disable these connectivity features.

[EXPLAIN TABLE]

\begin{table}
	\caption{[TABLE NEEDS RETESTING, MISSING DATA] Domains observed for connectivity check, remote provisioning, and location assistance.}\label{tab:domains-mechanisms}
	\centering
	\resizebox{\linewidth}{!}{%
		\begin{tabular}{|l|l|l|l|}
			...
			\hline
			Stock Android & connectivitycheck.gstatic.com & -- & agnss.goog\\
			LineageOS & connectivitycheck.gstatic.com & remoteprovisioning.googleapis.com & agnss.goog\\
			iodeOS & captiveportal.kuketz.de & remoteprovisioning.googleapis.com & --\\
			GrapheneOS & connectivitycheck.gstatic.com & remoteprovisioning.grapheneos.org & --\\
			\hline
		\end{tabular}%
	}
\end{table}
