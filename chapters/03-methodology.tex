...
We performed the following measurement tasks sequentially for each scenario. For each task, we captured network traffic using Wireshark for basic analysis.
...
Open preinstalled apps to perform simple actions, which should not require Internet connection:
\begin{itemize}
	\item \textbf{Settings:} disable haptic feedback; toggle dark mode; check per-app battery usage
	\item \textbf{Clock:} set an alarm
	\item \textbf{Files:} create a folder
	\item \textbf{Contacts:} create a local (on-device) contact
\end{itemize}